...
from craftpilot.grid.enums import CCW, DIR_VEC, OPPOSITE, BShape, Dir, Flag, Role
...
def _fix_stairs(grid: SemanticGrid) -> int:
    ...
    for x, y, z in zip(*np.nonzero(grid.block >= 0)):
        ...
        if info is None or int(grid.flags[x, y, z]) & Flag.FIXED_SHAPE:
            continue
...
def _fix_door_hinges(grid: SemanticGrid) -> None:
    ...
    for x, y, z in zip(*np.nonzero((grid.role == Role.DOOR) & (grid.shape == BShape.DOOR_LOWER))):
        ...
        left_ref = _ref_at(grid, x + lx, y, z + lz)
        # A double door: the right leaf hinges right so the pair opens apart.
        hinge = "left" if left_ref is not None and not left_ref.block_id.endswith("_door") else "right"
